...
exports._onNativeOrderPropertyChanged = childHandler;
...
var FlexboxLayout = (function (_super) {
    ...
    FlexboxLayout.prototype.addChild = function (child) {
        var index = this.getChildrenCount();
        this._reorderedIndices = this._createReorderedIndices(child, index, FlexboxLayout.getLayoutParams(child));
        _super.prototype.addChild.call(this, child);
    };
    FlexboxLayout.prototype.insertChild = function (child, index) {
        this._reorderedIndices = this._createReorderedIndices(child, index, FlexboxLayout.getLayoutParams(child));
        _super.prototype.addChild.call(this, child);
    };
    FlexboxLayout.prototype._createReorderedIndices = function (viewBeforeAdded, indexForViewBeforeAdded, paramsForViewBeforeAdded) {
        if (arguments.length === 0) {
            ...
            var childCount = this.getChildrenCount();
            var orders = this._createOrders(childCount);
            var orderForViewToBeAdded = new Order();
            ...
            if (indexForViewBeforeAdded === -1 || indexForViewBeforeAdded === childCount) {
                ...
                for (var i = indexForViewBeforeAdded; i < childCount; i++) {
                    orders[i].index++;
                }
            }
            else {
    ...
    FlexboxLayout.prototype._createOrders = function (childCount) {
        ...
        for (var i = 0; i < childCount; i++) {
            ...
            var params = FlexboxLayout.getLayoutParams(child);
            var order = new Order();
    ...
    Object.defineProperty(FlexboxLayout.prototype, "_isOrderChangedFromLastMeasurement", {
        get: function () {
            ...
            for (var i = 0; i < childCount; i++) {
                ...
                var lp = FlexboxLayout.getLayoutParams(view);
    ...
    FlexboxLayout.prototype._measureHorizontal = function (widthMeasureSpec, heightMeasureSpec) {
        ...
        (function () {
            var childCount = _this.getChildrenCount();
            ...
            for (var i = 0; i < childCount; i++) {
                ...
                var lp = FlexboxLayout.getLayoutParams(child);
                if (lp.alignSelf === flexbox_layout_common_1.AlignSelf.STRETCH) {
                    flexLine._indicesAlignSelfStretch.push(i);
                }
                var childWidth = lp.width;
                if (lp.flexBasisPercent !== flexbox_layout_common_1.FlexBasisPercent.DEFAULT && widthMode === EXACTLY) {
                ...
                if (_this._isWrapRequired(widthMode, widthSize, flexLine._mainSize, child.getMeasuredWidth() + lp.leftMargin + lp.rightMargin, lp, i, indexInFlexLine)) {
    ...
    FlexboxLayout.prototype._measureVertical = function (widthMeasureSpec, heightMeasureSpec) {
        ...
        for (var i = 0; i < childCount; i++) {
            ...
            if (lp.flexBasisPercent !== flexbox_layout_common_1.FlexBasisPercent.DEFAULT && heightMode === EXACTLY) {
            ...
            if (this._isWrapRequired(heightMode, heightSize, flexLine.mainSize, child.getMeasuredHeight() + lp.topMargin + lp.bottomMargin, lp, i, indexInFlexLine)) {
        ...
        for (var i = 0; i < flexLine.itemCount; i++) {
            ...
            if (this._isMainAxisDirectionHorizontal(flexDirection)) {
                if (!this._childrenFrozen[childIndex]) {
                    var rawCalculatedWidth = child.getMeasuredWidth() + unitSpace * lp.flexGrow + accumulatedRoundError;
                ...
                if (!this._childrenFrozen[childIndex]) {
                    var rawCalculatedHeight = child.getMeasuredHeight() + unitSpace * lp.flexGrow + accumulatedRoundError;
    ...
    FlexboxLayout.prototype._shrinkFlexItems = function (flexLine, flexDirection, maxMainSize, paddingAlongMainAxis, startIndex) {
        ...
        for (var i = 0; i < flexLine.itemCount; i++) {
            ...
            if (this._isMainAxisDirectionHorizontal(flexDirection)) {
                if (!this._childrenFrozen[childIndex]) {
                    var rawCalculatedWidth = child.getMeasuredWidth() - unitShrink * lp.flexShrink + accumulatedRoundError;
                    var roundedCalculatedWidth = Math.round(rawCalculatedWidth);
                ...
                if (!this._childrenFrozen[childIndex]) {
                    var rawCalculatedHeight = child.getMeasuredHeight() - unitShrink * lp.flexShrink + accumulatedRoundError;
                    var roundedCalculatedHeight = Math.round(rawCalculatedHeight);
        ...
        if (alignItems === flexbox_layout_common_1.AlignItems.STRETCH) {
            ...
            this._flexLines.forEach(function (flexLine) {
                for (var i = 0; i < flexLine.itemCount; i++, viewIndex_2++) {
                    ...
                    var lp = FlexboxLayout.getLayoutParams(view);
                    if (lp.alignSelf !== flexbox_layout_common_1.AlignSelf.AUTO && lp.alignSelf !== flexbox_layout_common_1.AlignSelf.STRETCH) {
                        continue;
                    }
                    switch (flexDirection) {
                        case flexbox_layout_common_1.FlexDirection.ROW:
    ...
    FlexboxLayout.prototype._isWrapRequired = function (mode, maxSize, currentLength, childLength, lp, childAbsoluteIndex, childRelativeIndexInFlexLine) {
    ...
    FlexboxLayout.prototype._layoutSingleChildHorizontal = function (view, flexLine, flexWrap, alignItems, left, top, right, bottom) {
        var lp = FlexboxLayout.getLayoutParams(view);
        if (lp.alignSelf !== flexbox_layout_common_1.AlignSelf.AUTO) {
...
var FlexboxLayout;
(function (FlexboxLayout) {
    function getLayoutParams(child) {
        ...
        var flp = {
            ...
            flexBasisPercent: flexbox_layout_common_1.FlexBasisPercent.DEFAULT,
    ...
    FlexboxLayout.getLayoutParams = getLayoutParams;
