...
vows.describe('prompt/interactive').addBatch({
  "When using prompt": {
    topic: function () {
      ...
      prompt.start();
      console.log('These prompt tests are interactive');
      console.log('Not following instructions will result in test failure');
      return null;
    },
    "the getInput() method": {
      "when passed a complex property with `hidden: true`": {
        topic: function () {
          console.log('When prompted, enter: 12345 [backspace] [backspace] [enter]');
        ...
        "and then when passed a complex property expecting a number": {
          topic: function () {
            console.log('When prompted, enter: 123 [enter]');
